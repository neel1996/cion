  ...
  render() {
    ...
    return (
      <ApolloProvider client={configuredItemGQLClient}>
        <div
          className={`container ${this.props.reference ||
            "configured-item-container"}`}
        >
          {
                  ...
                  <Query query={GET_CONFIG_ITEMS_QUERY}>
                    {({ loading, err, data }) => {
                      ...
                      if (data !== undefined) {
                        ...
                        return configDataResultArray.map(configDataResult => {
                          return (
                            <Link
                              to={`/configitem/${configDataResult.principleId}`}
                              key={configDataResult.principleId}
                            >
                              <div className="card-content card my-3 mr-5">
                                <img
                                  ...
                                  src={`${
                                    ...
                                    configDataResult.principleThumbnail.split(
                                      "/"
                                    )[
                                      configDataResult.principleThumbnail.split(
                                        "/"
                                  ...
                                  alt={configDataResult.principleName}
                                ></img>
                                <div className="card-body">
                                  <h3>{configDataResult.principleName}</h3>
                                  <p>{configDataResult.principleDescription}</p>
                                  <div className="float-left">
                                    <p className="btn btn-secondary" disabled>
                                      {configDataResult.totalItems} Items in set
                                    </p>
                                  </div>
                                </div>
                              </div>
